...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  const updUser = await User.findById(req.user._id).select(
    "-password -refreshToken"
  );
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  // from url we will gather information
  const {} = req.params;
  if (!username?.trim()) {
    throw new ApiError(400, "username is missing");
  }
  // can apply direct aggregation pipleine
  // values after aggregation comes as array
  const channel = await User.aggregate([
    ...
    },
    {
      $lookup: {
        from: "subscriptions",
        localField: "_id",
        // select channel and count to get subscribers of the channel
    ...
    {
      $lookup: {
        // this time we can how many subscribed by the channel
        from: "subscriptions",
        localField: "_id",
    ...
    // need to have additional fields(merging)
    {
      $addFields: {
        subscribersCount: {
          $size: "$subscribers",
        ...
        isSubscribed: {
          ...
          $cond: {
            ...
            // the document that we get as subscribers, we just want to check whether subscriber present
            // $in means whether present
    ...
    },
    {
      ...
      // selected things to be projected
    },
  ]);
...
const getWatchHistory = asyncHandler(async (req, res) => {
  // actually mongodb has a different expression of id , but mongoose converts it to required form
  const user = await User.aggregate([
    {
      $match: {
        // in aggregation pipelines mongoose doesnt work so dont write req.user._id, need to create mongoose object id
    ...
    },
    {
        ...
        pipeline: [
          {
            $lookup: {
              ...
              pipeline: [
                {
                  // need to project certain things only
                  $project: {
                    fullName: 1,
                    username: 1,
                    avatar: 1,
                ...
                {
                  // because after lookup we get array
                  // so pick first part from it
      ...
      // now there will be lots of documents but in videos there is owner as well need to get it as well using sub pipeline
    },
  ]);
